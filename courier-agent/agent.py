...
# how long the agent waits with no messages before exiting
MAX_IDLE_CYCLES = 12   # 12 x 10s = about 2 minutes idle
...
if not SB_QUEUE:
    raise RuntimeError("SERVICE_BUS_QUEUE missing")

if not STORAGE_CONN:
    raise RuntimeError("AZURE_STORAGE_CONNECTION_STRING missing")

if not TABLE_NAME:
    raise RuntimeError("AZURE_TABLE_NAME missing")
...
def get_eta(origin_coord, dest_coord, route_client):
    ...
    if matrix_cell.status_code != 200:
        raise RuntimeError(f"Route error: {matrix_cell.response}")
...
def load_factories():
    ...
    with open(BASE_DIR / "factories.csv", newline="", encoding="utf-8") as csvfile:
    ...
    logger.info("Loaded %d active factories", len(factories))
    return factories
...
def find_closest_factory(origin_coord, factories, route_client):
    ...
    for factory in factories:
        factory_coord = [factory["Longitude"], factory["Latitude"]]

        eta = get_eta(origin_coord, factory_coord, route_client)
        logger.info("Factory %s ETA: %ss", factory["FactoryName"], eta)

        if shortest_eta is None or eta < shortest_eta:
            shortest_eta = eta
            closest_factory = factory

    if closest_factory is None:
        raise RuntimeError("No active factory could be selected.")
...
def process_message(msg_body, search_client, route_client, table_client):
    ...
    if req.get("Status", "").strip().lower() != "approved":
        logger.info("Skipping non-approved request: %s", req.get("Status"))
        return

    required_fields = ["PartitionKey", "RowKey", "PickupAddress"]
    ...
    closest_factory, eta_out = find_closest_factory(origin, factories, route_client)

    destination = [closest_factory["Longitude"], closest_factory["Latitude"]]
    logger.info("Assigned factory: %s", closest_factory["FactoryName"])
...
def main():
    ...
    try:
        with sb_client:
            receiver = sb_client.get_queue_receiver(queue_name=SB_QUEUE, max_wait_time=POLL_WAIT_SECONDS)
            with receiver:
                while True:
                    if STOP_FILE.exists():
                    ...
                    try:
                        msgs = receiver.receive_messages(max_message_count=1, max_wait_time=POLL_WAIT_SECONDS)
                    ...
                    if not msgs:
                        idle_cycles += 1
                        logger.info("No messages received. Idle cycle %d/%d", idle_cycles, MAX_IDLE_CYCLES)

                        if idle_cycles >= MAX_IDLE_CYCLES:
                            logger.info("Queue has been idle long enough. Shutting down.")
                            break
